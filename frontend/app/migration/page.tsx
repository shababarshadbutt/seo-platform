                        ...
                        <>
                          <select
                            ...
                            onChange={(event) => {
                              ...
                              setSftpDomain(nextDomain);
                              setFormError("");
                              // Base URL is FILLED FROM THE SELECTION rather than
                              // typed. It used to be left blank for manual entry,
                              // and a hand-typed www variant is exactly how one
                              // domain's sitemaps ended up published under a
                              // second, wrong S3 prefix. Filling it from the same
                              // value that becomes sessions.sftp_domain means the
                              // two start out in agreement instead of hoping.
                              setBaseUrl(baseUrlFromSftpDomain(nextDomain));
                              // Not "touched" by the user — suppress the
                              // validation error a programmatic fill would
                              // otherwise trip on an empty selection.
