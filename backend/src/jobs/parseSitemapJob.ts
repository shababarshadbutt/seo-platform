...
export async function processParseSitemapJob(
  ...
  try {
    ...
    // Whatever went wrong above — including a transient failure in the very
    // first DB call (isSessionCancelled), which is exactly how this used to
    // fail in production: a momentary "getaddrinfo EAI_AGAIN postgres" during
    // a container restart left a row parsed_at IS NULL forever, because nothing
    // downstream of an uncaught throw here ever touched it again, and the
    // fixed-jobId re-enqueue the watchdog relies on was a silent no-op against
    // the now-permanently-"failed" BullMQ job (see enqueueParseSitemapJobs).
    // `file` may not have been resolved yet, so use the input ids directly —
    // and only touch the row if it isn't already terminal, so a failure AFTER
    // a successful COMMIT (e.g. enqueueing child jobs) can't clobber a parse
    ...
    try {
      await pool.query(
        `
          UPDATE sitemap_files
          SET parsed_at = NOW(), is_valid = FALSE, parse_error = $2
          WHERE id = $1 AND parsed_at IS NULL
        `,
        [data.sitemap_file_id, message.slice(0, 2000)]
      );
    } catch (writeError) {
      logger.error(
        { sitemap_file_id: data.sitemap_file_id, err: writeError },
        "failed to record parse failure for sitemap file"
      );
    }

    try {
      if (!(await isSessionCancelled(data.session_id))) {
        const parsedCount = await incrementParsedSitemapCount(data.session_id);
        await tryFinalizeParsedSession(
          data.session_id,
          logger,
          parsedCount,
          data.sitemap_file_id
        );
      }
    } catch (finalizeError) {
      logger.error(
        ...
        "failed to finalize session after parse failure"
      );
    }
